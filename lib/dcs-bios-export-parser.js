...
class DcsBiosExportParser extends EventEmitter {
          ...
          this._addressLookup[controlAddress].forEach((control) => {
            control.outputs.forEach((output) => {
              ...
              if (valueHasChanged && !isAlreadyAdded) {
                // Save the current and the previous values on the output object.
  ...
  _processEmitQueue() {
    ...
    this._emitQueue.forEach((item) => {
      ...
      this.emit(identifier + ':' + item.output.value, item.control, item.output);
      // Emit the transition between two values, i.e. 'MASTER_CAUTION:0->1'
      if (item.output.previousValue != item.output.value) {
        this.emit(identifier + ':' + item.output.previousValue + '->' + item.output.value, item.control, item.output);
      }
    });
